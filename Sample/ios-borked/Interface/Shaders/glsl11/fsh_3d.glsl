...
varying highp vec3 position_eye, normal_eye;
varying highp vec2 texture_coordinates;
varying highp float fragment_depth;
...
uniform mediump sampler2D basic_texture;
//Fog data
...
vec4 ApplyLight(vec3 intensity, vec3 color, vec4 components, vec3 position) {
	highp float specular_exponent = 20.0/components.w;
	highp vec3 Ls = color;
	highp vec3 Ld = color;
	highp vec3 La = color;
	...
	// surface reflectance
	highp vec3 Ks = vec3 (1.0, 1.0, 1.0)*(components.z*material_reflectivity.x); // fully reflect specular light
	highp vec3 Kd = vec3 (1.0, 1.0, 1.0)*components.y; // orange diffuse surface reflectance
	highp vec3 Ka = vec3 (1.0, 1.0, 1.0)*components.x; // fully reflect ambient light
	...
	highp vec3 light_position_eye = vec3(vView * vec4(position, 1.0));
	highp vec3 distance_to_light_eye = light_position_eye - position_eye;
	highp vec3 direction_to_light_eye = normalize (distance_to_light_eye);
	highp float dot_prod = dot (direction_to_light_eye, normal_eye);
	dot_prod = max (dot_prod, 0.0);
	highp vec3 Id = Ld * Kd * dot_prod; // final diffuse intensity
	...
	highp float dist = sqrt(dot(distance_to_light_eye,distance_to_light_eye));
	highp float lightStrength = max(0.0,1.0 - intensity.x - intensity.y*dist - intensity.z*dist*dist);

	
	highp vec3 Is = vec3(0,0,0);
	// specular intensity
	if ((Ks.z > 0.0) && (lightStrength > 0.0)) {
		highp vec3 reflection_eye = reflect (-direction_to_light_eye, normal_eye);
		highp vec3 surface_to_viewer_eye = normalize (-position_eye);
		highp float dot_prod_specular = dot (reflection_eye, surface_to_viewer_eye);
		dot_prod_specular = max (dot_prod_specular, 0.0);
		highp float specular_factor = pow (dot_prod_specular, specular_exponent * max(.1,material_reflectivity.y));
		Is = Ls * Ks * specular_factor; // final specular intensity 
	} 
	
	//Combine into lighted color
	highp vec4 lightedPixel = vec4(Is + Id + Ia, 1.0);
...
void main () {
    ...
    highp vec4 fcolor = vec4(0,0,0,0);
	...
	highp vec4 texel = texture2D(basic_texture, texture_coordinates);
	fcolor.w = 1.0;
	texel.w = 1.0;
	highp vec4 finalColor = fcolor*texel;
	
	//Handle fog now
	highp float partDistance = (maxDistance/4.0);
	//Calculate how much fog should effect the current pixel
	highp float fogFactor = max(min((fragment_depth-partDistance*3.0)/partDistance,1.0),0.0);
