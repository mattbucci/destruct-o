...
void main () {
	...
	//texel = vec4(texture_coordinates.x,texture_coordinates.y,0.0,1.0);
	fcolor.w = 1.0;
	texel.w = 1.0;
	
	vec4 finalColor = fcolor*texel;
	
	//Handle fog now
	float partDistance = (maxDistance/4.0);
	//Calculate how much fog should effect the current pixel
	float fogFactor = max(min((fragment_depth-partDistance*3.0)/partDistance,1.0),0.0);
